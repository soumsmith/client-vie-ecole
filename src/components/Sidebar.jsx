...
// Fonction pour filtrer les sections selon le profil
const filterSectionsByProfile = (sections, userProfile) => {
  return sections.filter((section) => {
    ...
    const filteredItems = section.items.filter((item) => {
      ...
      if (!isItemAllowed) return false;

      if (item.type === "menu" && item.children && isItemAllowed) {
        const filteredChildren = item.children.filter((child) => {
          return !child.profiles || child.profiles.includes(userProfile);
        });
